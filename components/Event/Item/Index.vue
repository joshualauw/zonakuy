<template>
    <div class="card relative flex flex-row items-start md:space-x-6 w-full h-52 mb-8">
        <NuxtImg class="w-52 hidden md:block h-full rounded-lg" src="/img/default_event.jpg"></NuxtImg>
        <div class="w-3/4 h-full">
            <h1 class="font-bold text-base md:text-lg mb-1">{{ data.name }}</h1>
            <p class="text-sm">
                <Icon name="material-symbols:timer" class="text-gray-500 mr-2"></Icon
                >{{ dayjs(data.start_date).format("ddd, D MMM YYYY") }} -
                {{ dayjs(data.end_date).format("ddd, D MMM YYYY") }}
            </p>
            <p class="text-sm">
                <Icon name="material-symbols:location-on" class="text-gray-500 mr-2 mb-1"></Icon
                >{{ data.location ?? "-" }}
            </p>
            <ElTag v-for="tag in data.tags" class="m-1.5 cursor-pointer hover:brightness-90" type="info">{{
                tag
            }}</ElTag>
        </div>
        <div class="divider divider-horizontal"></div>
        <div class="w-1/4 h-full">
            <EventItemJoinAction v-if="!type || type == 'join'" />
            <EventItemLinkAction v-else-if="type == 'link'" />
            <EventItemEditAction v-else :edit-link="`/event/${data.slug}/dashboard/report`" />
        </div>
    </div>
</template>

<script setup lang="ts">
//TODO: make event item action slot
import dayjs from "dayjs";
import { Event } from "@prisma/client";

defineProps<{ data: Event; type: "join" | "link" | "edit" }>();
</script>
