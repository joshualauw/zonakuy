<template>
    <div class="card relative flex flex-row items-start md:space-x-6 w-full h-52 mb-8">
        <NuxtImg class="w-52 hidden md:block h-full rounded-lg" :src="event.banner ?? '/img/default_event.jpg'" />
        <div class="w-3/4 h-full">
            <h1 class="font-bold text-base md:text-lg mb-1">{{ event.name }}</h1>
            <p class="text-sm">
